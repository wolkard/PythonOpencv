...
import sys
import json
sys.path.append("/home/renbo/engRec")
import src.predict_change
...
def get_good_img(img,close_y,close_x,open_y,open_x):#
    ...
    #cv2.imwrite("/home/renbo/web/static/debug_img/"+str(name)+'g.jpg', close)
    return open_img
...
def get_column(img,first_row,second_row):
    img = get_good_img(img[int(first_row):int(second_row),:],100,10,20,10)#30,12,6,6
    #name=time.time()
    #cv2.imwrite("/home/renbo/web/static/debug_img/col_"+str(name)+'g.jpg', img)
    y = img.mean(axis=0)
...
def run(img):
    good_img = get_good_img(img,5,40,5,5)
